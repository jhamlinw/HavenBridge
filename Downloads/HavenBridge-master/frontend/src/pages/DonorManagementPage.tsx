...
export default function DonorManagementPage() {
  ...
  const totalPages = Math.ceil(supporters.length / DONORS_PER_PAGE);
  ...
  return (
    <div className="max-w-[1600px] mx-auto px-6 py-10">
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100">
                <p className="text-xs text-gray-500">
                  {(currentPage - 1) * DONORS_PER_PAGE + 1}–{Math.min(currentPage * DONORS_PER_PAGE, supporters.length)} of {supporters.length}
                </p>
                <div className="flex items-center gap-1">
                  <button
                    onClick={() => setCurrentPage(p => Math.max(1, p - 1))}
                    disabled={currentPage === 1}
                    className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
                  >
                  ...
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
                    <button
                      ...
                      className={`w-7 h-7 rounded-lg text-xs font-medium transition-all ${
                        page === currentPage ? 'bg-haven-600 text-white' : 'text-gray-600 hover:bg-gray-100'
                      }`}
                    >
                      {page}
                    </button>
          ...
          {!selected ? (
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => setShowFlagConfirm(true)} className="flex-1 border border-red-200 text-red-700 text-sm font-medium py-2.5 rounded-xl hover:bg-red-50 transition-all">
                  Flag At-Risk
                </button>
              </div>
            </div>
      ...
      <ConfirmDialog
        ...
        title="Flag donor as At-Risk?"
        message={`This will mark ${selected?.displayName ?? 'this donor'} as At-Risk. Are you sure?`}
        confirmLabel="Flag At-Risk"
        onConfirm={() => {
          // TODO: call API to update donor status when endpoint is available
