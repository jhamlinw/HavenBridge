    ...
    <div className="min-h-screen flex flex-col bg-gray-50/60">
      <header className="bg-white/80 backdrop-blur-md border-b border-gray-200/70 sticky top-0 z-30 shadow-sm">
        <div className="max-w-[1600px] mx-auto flex items-center h-16 px-6">
          {/* Logo */}
          <NavLink to={hasRole('Staff') ? '/dashboard' : '/donor-portal'} className="flex items-center gap-2.5 mr-6 md:mr-10 shrink-0">
          ...
          {/* Mobile hamburger */}
          <button
            className="md:hidden ml-auto p-2 rounded-lg text-gray-600 hover:bg-gray-100 transition-all"
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-gray-100 bg-white px-6 py-4 space-y-1">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                    isActive ? 'bg-haven-50 text-haven-700' : 'text-gray-700 hover:bg-gray-50'
                  }`
                }
              >
                <Icon className="h-5 w-5" />
            ...
            <div className="pt-3 border-t border-gray-100 mt-3">
              <p className="text-xs text-gray-400 px-4 mb-2">{username}{role ? ` (${role})` : ''}</p>
              <button
                onClick={handleLogout}
